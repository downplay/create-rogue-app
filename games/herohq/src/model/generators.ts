...
export const populateRoom: RoomGenerator = ({ room, rng, level }) => {
    ...
    for (let n = 0; n < mobCount; n++) {
        ...
        entities.push({
            type: BugMonster,
            params: { power: Math.max(1, Math.round(level + dist() * 5 - 2)) }
        })
    }
    return entities
}
...
export const populateRoom = (props: RoomGeneratorProps) => {
    ...
    for (const a of actors) {
        ...
        set(actorFamily(id), {type:"Initialize", params: a.params}))
    }
}
