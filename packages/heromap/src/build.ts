...
import { VirtualGrid, VirtualGridCell, virtualGrid } from "./virtualGrid"
...
const distributeTargetsToNodes = (
    ...
    const fractionAdjust = remainingFraction < 0 && totalFraction > 0 ? 1 / totalFraction : 1
    ...
    for (const target of targets) {
        const picked =
            weightedList.length === 1 ? weightedList[0] : context.rng.pick(weightedList, "weight")
        if (picked.node) {
            const found = brushesToTargets.find(({ brush }) => brush === picked.node.brush)
...
const applyBrushToGlyphs = (glyphs: string[], brush: BrushNode, context: BuildContext) => {
...
export type MapResult = {
    ...
    cells: VirtualGridCell<MapElement[]>[]
    raw: String[][]
}

export const build = <E = any>(
    map: MapNode<E>,
    rng: RNG = createRng(),
    scope: Record<string, any> = {}
): MapResult => {
    ...
    return {
        ...
        // TODO: And honestly why are rows in the bounds?
        bounds: {
            x: 0,
            y: 0,
            width: grid.width,
