...
export const virtualGrid = <T>(width: number = 0, height: number = 0): VirtualGrid<T> => {
    ...
    const update = (x: number, y: number, updater: (current: T | undefined) => T | undefined) => {
        const current = get(x, y)
        set(x, y, updater(current))
    }

    const getAll = () => Object.values(internalGrid).flatMap((row) => Object.values(row))

    // TODO: Kinda temporary to support old implementation of map, remove once deprecated
    const getRows = () =>
        Object.values(internalGrid).map((row) => Object.values(row).map((cell) => cell.element))
