...
const executeChoicesNode = (
    ...
    if (strand.children[0]) {
        ...
        const filtered = choices.filter((choice) => matchPreconditions(choice, context, strand))
...
const resolveLabelPath = (
    ...
    if (childStrand.path === "label") {
        if (typeof path === "object") {
            ...
            if (isSuspended(childStrand)) {
                ...
                return [strand.internalState, resolveParent, [labelResult[labelResult.length - 1]]]
            } else {
                strand.internalState = (strand.internalState || "") + stringifyResult(labelResult)
            }
        }
    }
    ...
    const labelName = strand.internalState ? (strand.internalState as string) : (path as string)
    ...
    if (resolveLabels && Object.prototype.hasOwnProperty.call(strand.localScope, labelName)) {
...
const executeFunctionNodeInvocation = (
    ...
    if (!node.signature || node.signature.length === 0) {
        throw new Error(
            "Label " + node.name + " has no parameters so cannot be called as a function"
        )
    }
    ...
    for (const i in node.signature) {
        if (!node.signature[i].optional && typeof parameters[i] === "undefined") {
            throw new Error("Label " + node.name + " requires parameter $" + node.signature[i].name)
        }
    }
...
const executeSubstitutionNode = (
    ...
    let resolveParent: any =
        typeof strand.internalState === "undefined" ? context.state : strand.internalState
...
const executeAssignmentNode = (
    ...
    if (isSuspended(childStrand)) {
        ...
        context.state[node.variable] = coalesceResult([...(strand.internalState || []), ...result])
...
const executeInputNode = (node: InputAST, context: ExecutionContext, strand: ExecutionStrand) => {
...
executeNode = (
    ...
    if (typeof node === "string") {
        ...
        switch (node.type) {
            ...
            case "substitution":
            case "invoke":
                return executeSubstitutionNode(node as ContentSubstitutionAST, context, strand)
            case "assignment":
                return executeAssignmentNode(node as ContentAssignmentAST, context, strand)
            case "external":
            ...
            default:
                throw new Error("Unknown node type in node: " + JSON.stringify(node, null, "  "))
        }
    }
}
